...
namespace vkrollercoaster {
    ...
    public:
        template <typename T>
        vertex_buffer(const std::vector<T>& data)
            : vertex_buffer(data.data(), data.size() * sizeof(T)) {}
    ...
    public:
        static ref<uniform_buffer> from_shader_data(ref<shader> _shader, uint32_t set,
                                                    uint32_t binding);
