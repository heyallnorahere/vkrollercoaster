...
namespace vkrollercoaster {
    ...
    void command_buffer::end() {
        ...
        this->m_recorded = true;
    }

    // todo: more parameters i guess?
    void command_buffer::submit() {
        if (!this->m_recorded) {
            throw std::runtime_error("cannot submit an unrecorded command buffer!");
        }

        if (this->m_recording) {
            throw std::runtime_error("cannot submit a command buffer that is currently recording!");
        }
    ...
    void command_buffer::end_render_pass() {
        if (!this->m_recording) {
            throw std::runtime_error("cannot end a render pass while not recording!");
        }
